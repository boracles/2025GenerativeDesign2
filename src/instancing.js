...
export async function initBoidsSystem({
  ...
  const W_COH = 0.5; // Cohesion (가까워짐)
  const W_ALI = 0.45; // Alignment (방향 정렬)
  const W_PULL = 1.0; // External Pull (외부 자극)
  const W_VTX = 0.8; // Vortex (소용돌이)
  const VORTEX_THRESHOLD = 0.9; // 밀도 임계값 (ρ)

  const MAX_FORCE = 0.02; // 한 프레임당 최대 조향력
  const MAX_SPEED = 0.35; // 최대 속도
  const DAMPING = 0.96; // 감쇠 (마찰)
  ...
  const WORLD_RADIUS = worldRadius;
  const WALL_WIDTH = 6; // 경계 완충 폭
  const WALL_K = 0.035; // 벽 반발 세기

  // 개인 공간(Separation 영역)
  const SEP_R = Math.min(GAP_X, GAP_Z) * 0.55 * 0.25;
  ...
  const PULL_GAIN = 1.0; // 외부 자극(−∇ρ) 증폭
  ...
  function updateBoids(dt, tSec) {
    ...
    for (let i = 0; i < COUNT; i++) {
      ...
      const SPEED_K = 0.7; // 전역 감속 노브(0~1)
      positions[i].x += vel[i].x * dt * SPEED_K;
      positions[i].z += vel[i].z * dt * SPEED_K;

      // ── (4.5) 경계 하드-클램프 + 반사/감쇠
      if (bounds) {
        const b = bounds;
        const bounce = 0.4; // 반사 감쇠(0~1), 0.4면 60% 속도 손실
        if (positions[i].x < b.minX) {
          positions[i].x = b.minX;
          if (vel[i].x < 0) vel[i].x = -vel[i].x * bounce;
        } else if (positions[i].x > b.maxX) {
          positions[i].x = b.maxX;
          if (vel[i].x > 0) vel[i].x = -vel[i].x * bounce;
        }
        if (positions[i].z < b.minZ) {
          positions[i].z = b.minZ;
          if (vel[i].z < 0) vel[i].z = -vel[i].z * bounce;
        } else if (positions[i].z > b.maxZ) {
          positions[i].z = b.maxZ;
          if (vel[i].z > 0) vel[i].z = -vel[i].z * bounce;
        }
      }

      if (typeof sampleHeight === "function") {
        positions[i].y = sampleHeight(positions[i].x, positions[i].z) || 0;
      }

      const root = objects[i];
      if (!root) continue; // GLB 준비 전이면 스킵
  ...
  function reselectionPass(init = false) {
    ...
    for (const i of ids) {
      const p = positions[i];
      const d2 =
        (p.x - camera.position.x) ** 2 + (p.z - camera.position.z) ** 2;
  ...
  const gltf = await loader.loadAsync(GLB_PATH);
  {
    ...
    for (let i = 0; i < COUNT; i++) {
      ...
      const y = typeof sampleHeight === "function" ? sampleHeight(p.x, p.z) : 0;
      p.y = y;
      root.position.set(p.x, p.y, p.z);

      root.rotation.set(0, Math.random() * Math.PI * 2, 0);
      const AGENT_SCALE = 0.25;
  ...
  function wallForce(pi) {
    if (!bounds) return new THREE.Vector2(0, 0);
    const m = 0.75; // 벽 완충폭(월드 유닛) – 취향대로
    const k = WALL_K; // 벽 힘 계수(기존 변수 재사용)
    let fx = 0,
      fz = 0;
    // 좌/우
    const dl = pi.x - (bounds.minX + m);
    const dr = bounds.maxX - m - pi.x;
    if (dl < 0) fx += -dl * k; // 왼쪽 벽 안쪽으로
    if (dr < 0) fx -= -dr * k; // 오른쪽 벽 안쪽으로
    // 앞/뒤
    const db = pi.z - (bounds.minZ + m);
    const df = bounds.maxZ - m - pi.z;
    if (db < 0) fz += -db * k; // 뒤(−z) 벽
    if (df < 0) fz -= -df * k; // 앞(+z) 벽
  ...
  function update(dt, tSec) {
    tryApplyRDToPending();
    reselectionPassIfNeeded();
    updateDisturbanceField(dt);
    if (DEBUG_SENSE) {
      updateNeighborRings();
      updateSenseArrows(tSec);
    }
    updateBoids(dt, tSec);
    // 근거리 스킨 애니
    animSet.forEach((idx) => {
      ...
      if (act) {
        ...
        const jitter =
          1.0 + 0.12 * Math.sin(tSec * jitterSpeed[idx] + jitterPhase[idx]);
        act.timeScale = base * jitter;
      }
      mx.update(dt);
    });
    // 원거리 인스턴스 지터(기존 코드 그대로)
    // ... (instBody/instLegBall/instLegStick 업데이트 블록 유지)
  }

  function reselectionPassIfNeeded() {
    // 카메라 이동/주기적 재선정 로직만 추출
    if (!reselectionPassIfNeeded.__init) {
      reselectionPass(true);
      reselectionPassIfNeeded.__init = true;
      reselectionPassIfNeeded._frame = 0;
      reselectionPassIfNeeded._prev = new THREE.Vector3().copy(camera.position);
    }
    const prev = reselectionPassIfNeeded._prev;
    const moved = camera.position.distanceToSquared(prev) > MOVE_EPS * MOVE_EPS;
    if (moved || reselectionPassIfNeeded._frame++ % RESELECT_EVERY === 0) {
      reselectionPass();
  ...
  function dispose() {
    [instBody, instLegBall, instLegStick, ringInst].forEach((m) => {
      if (m) {
        scene.remove(m);
        m.geometry.dispose();
        m.material.dispose();
    ...
    mixers.forEach((mx) => mx.uncacheRoot && mx.uncacheRoot());
  }

  return { update, addEmitter, reselectionPass, dispose };
}
