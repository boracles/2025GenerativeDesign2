...
import { initBoidsSystem } from "./instancing.js";
...
function sampleHeightWorld(x, z) {
  if (!heightSampler) return 0;
  // 월드 → 텍스처 좌표
  const u = THREE.MathUtils.clamp(x / SIZE + 0.5, 0, 1);
  const v = THREE.MathUtils.clamp(z / SIZE + 0.5, 0, 1);
  const ix = Math.min(SIM_SIZE - 1, Math.max(0, (u * SIM_SIZE) | 0));
  const iy = Math.min(SIM_SIZE - 1, Math.max(0, (v * SIM_SIZE) | 0));
  return heightSampler(ix, iy) * params.disp; // 0..1 → 월드 y
}
...
const colorMat = new THREE.ShaderMaterial({
  uniforms: {
    ...
    heightTexSharp: { value: heightRT.texture },
...
function bake(dt) {
  ...
  heightSampler = readHeightToSampler();
}
...
(async () => {
  // 최초 1회 bake 후 sampler가 생김 (이미 위에서 bake() 호출함)
  if (!heightSampler) heightSampler = readHeightToSampler();

  boids = await initBoidsSystem({
    scene,
    camera,
    renderer,
    sampleHeight: sampleHeightWorld,
    worldRadius: SIZE * 0.5, // 경계 반경 = 지형 반폭
    count: 120,
    bounds: {
      minX: -SIZE * 0.5,
      maxX: SIZE * 0.5,
